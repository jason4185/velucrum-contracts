...
async function main() {
  ...
  // Seed yield source with 10,000 cUSDT
  await (await cusdt.approve(ysAddr, ethers.parseUnits("10000", 6))).wait();
  await (await yieldSource.seedYield(ethers.parseUnits("10000", 6))).wait();
  console.log("✅ Yield source seeded with 10,000 cUSDT");
